...
insert into reports (category, description, latitude, longitude, neighborhood, status, session_id, created_at) values
  ...
  -- Second wave (2026-07-15): cover the remaining categories so the ledger and
  -- the category filter chips all have something to show during the walkthrough.
  ('street_lighting', 'Sokak lambaları üç gündür yanmıyor, akşam saatlerinde cadde tamamen karanlık.',           37.041, 35.288, 'Toros',           'open',     'seed-demo', now() - interval '6 days'),
  ('water_sewage',    'Yağmur sonrası rögar taşıyor, kaldırım su altında kalıyor.',                               36.986, 35.325, 'Ziyapaşa',        'open',     'seed-demo', now() - interval '12 days'),
  ('stray_animals',   'Boş arsada sahipsiz köpekler toplanıyor; hemen yanında çocuk parkı var.',                  37.048, 35.263, 'Belediye Evleri', 'open',     'seed-demo', now() - interval '4 days'),
  ('noise',           'Gece 23:00''ten sonra da süren inşaat gürültüsü bildirildi; çalışma saatleri düzenlendi.', 36.995, 35.345, 'Reşatbey',        'resolved', 'seed-demo', now() - interval '18 days'),
  ('cleanliness',     'Pazar sonrası meydan temizlenmemişti; ertesi gün ekipler temizledi.',                      36.985, 35.328, 'Tepebağ',         'resolved', 'seed-demo', now() - interval '8 days');
...
-- Second wave: the two resolved rows get a backing 'resolved' confirmation
-- (matching the app's community-resolve flow), the lighting row two 'still_open'.
insert into confirmations (report_id, type, session_id)
select id, 'resolved', 'demo-c4' from reports
where session_id = 'seed-demo' and neighborhood in ('Reşatbey', 'Tepebağ');

insert into confirmations (report_id, type, session_id)
select r.id, 'still_open', s.sess
from reports r
cross join (values ('demo-c5'), ('demo-c6')) as s(sess)
where r.session_id = 'seed-demo' and r.neighborhood = 'Toros';
