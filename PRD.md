...
| **Settings (Görünüm)** | Display & accessibility preferences, persisted per-device (no account): theme (system/light/dark — a "night ledger" dark mode), high-contrast, text size, reduced motion. Applied via CSS variables; a no-flash script restores the choice before paint. Added 2026-07-12. |
