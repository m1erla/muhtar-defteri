...
- **Adana only.** Never add multi-city logic, city selectors, or "expand to other provinces" scaffolding unless explicitly asked. This is the single most common way this project accidentally grows out of scope. Routing channels are Adana-specific and hand-verified; a province selector would misroute other provinces to Adana's hotlines. The location hierarchy stops at **district** (İlçe) — Adana's 15 districts live in `lib/adana-districts.ts` (the one data-driven source), used by the accessible `Combobox` on report-details. **The map must never be the only way to set a location** (the district selector is the keyboard/screen-reader path); map moves must stay reduced-motion-safe (instant `setView`).
